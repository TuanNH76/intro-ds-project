...
import pandas as pd
import numpy as np  # Added import for handling NaT values
...
def update_social_features():
    ...
    df = load_collection_to_dataframe("mentioned_frequency")
    ...
    print(f"Processing social features for {len(unique_datetimes)} timestamps and {len(COIN_SYMBOLS)} coins...")
    ...
    for coin in COIN_SYMBOLS:
        ...
        for current_time in unique_datetimes:
            ...
            # Calculate features
            mentioned_6h = sum([r['mentioned_frequency'].get(coin, 0) for _, r in lookback_rows.iterrows()])
            mentioned_24h = sum([r['mentioned_frequency'].get(coin, 0) for _, r in day_lookback_rows.iterrows()])
