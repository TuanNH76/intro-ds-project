...
async def integrate_coin_data(coin_symbol):
    """
    ...
    try:
        technical_df = await mongo_client.load_collection_to_dataframe(
    ...
    try:
        social_df = await mongo_client.load_collection_to_dataframe(social_collection)
        if social_df.empty:
    ...
    for hours in [1, 3, 6, 12, 24]:
        ...
        technical_df[f"future_price_{hours}h"] = technical_df["close_price"].shift(
            -hours
        )
    ...
    if social_df is not None:
        ...
        if "mentioned" in merged_df.columns and merged_df["mentioned"].sum() > 0:
            merged_df["price_to_mention"] = merged_df["close_price"] / merged_df[
                "mentioned"
            ].replace(0, 0.1)
            merged_df["volume_to_mention"] = merged_df["volume_to"] / merged_df[
                "mentioned"
            ].replace(0, 0.1)
    ...
    merged_df = merged_df.dropna(
        subset=[f"price_change_{hours}h" for hours in [1, 3, 6, 12, 24]]
    )
    ...
    await mongo_client.save_dataframe_to_collection(merged_df, output_collection)

    print(
        f"Successfully integrated data for {coin_symbol}. {len(merged_df)} records saved to '{output_collection}' collection."
    )

    return merged_df


async def integrate_all_coins():
    """
    ...
    for coin in COIN_SYMBOLS:
        try:
            df = await integrate_coin_data(coin)
            if df is not None:
                results[coin] = len(df)
            else:
...
if __name__ == "__main__":
    ...
    res = asyncio.run(integrate_all_coins())
