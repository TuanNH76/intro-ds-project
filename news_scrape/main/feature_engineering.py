...
from ta.trend import MACD, ADXIndicator, AroonIndicator, SMAIndicator
...
def calculate_technical_indicators(df):
    ...
    def calculate_obv(df):
        ...
        for i in range(1, len(df)):
            if (
                df["close_price"].iloc[i] > df["close_price"].iloc[i - 1]
            ):  # price increase
    ...
    def calculate_ad_line(df):
        ...
        for i in range(1, len(df)):
            ...
            if df["high_price"].iloc[i] == df["low_price"].iloc[i]:
                ...
                mf_multiplier = (
                    (df["close_price"].iloc[i] - df["low_price"].iloc[i])
    ...
    result_df["adx"] = ADXIndicator(
        result_df["high_price"],
        result_df["low_price"],
        result_df["close_price"],
        window=14,
    ).adx()

    # 4. Calculate Aroon Indicator (Aroon Up, Aroon Down)
    aroon = AroonIndicator(
        high=result_df["high_price"],
        low=result_df["low_price"],
        window=14,
        fillna=False,
    )
    ...
    result_df["bb_width"] = (result_df["bb_upper"] - result_df["bb_lower"]) / result_df[
        "bb_middle"
    ]

    # 9. Calculate Simple Moving Averages
    result_df["sma_7"] = SMAIndicator(
        result_df["close_price"], window=7
    ).sma_indicator()
    result_df["sma_20"] = SMAIndicator(
        result_df["close_price"], window=20
    ).sma_indicator()
    result_df["sma_50"] = SMAIndicator(
        result_df["close_price"], window=50
    ).sma_indicator()
    result_df["sma_200"] = SMAIndicator(
        result_df["close_price"], window=200
    ).sma_indicator()

    return result_df


async def process_coin(coin_symbol, update_only=False):
    """
    ...
    if update_only:
        ...
        latest_timestamp = await mongo_client.get_latest_timestamp(
        ...
        if latest_timestamp:
            ...
            # Execute query
            new_docs = await mongo_client.find_documents(raw_collection_name, query)
            ...
            print(
                f"No existing data found for {coin_symbol} in {output_collection_name}. Processing all data."
            )
            raw_df = await mongo_client.load_collection_to_dataframe(
                raw_collection_name
            )
    else:
        # Load all raw data
        raw_df = await mongo_client.load_collection_to_dataframe(raw_collection_name)
    ...
    if update_only and latest_timestamp:
        ...
        historical_query = {
            "datetime": {
                ...
                - timedelta(days=30),  # Approximately 30 days should be enough
            }
        }

        historical_docs = await mongo_client.find_documents(
            raw_collection_name, historical_query
        )
        ...
        if historical_docs:
            ...
            combined_df = (
                pd.concat([historical_df, cleaned_df])
    ...
    # Save to MongoDB
    await mongo_client.save_dataframe_to_collection(
        processed_df, output_collection_name
    )

    print(
        f"Completed processing {coin_symbol}. {len(processed_df)} records saved to '{output_collection_name}' collection."
    )

    return processed_df


async def update_feature_engineering():
    """
    ...
    for coin in COIN_SYMBOLS:
        try:
            result_df = await process_coin(coin, update_only=True)
    ...
    for coin, result in update_results.items():
        print(
            f"{coin}: {result} new records processed"
...
async def process_all_coins():
    ...
    for coin in COIN_SYMBOLS:
        try:
            _ = await process_coin(coin, update_only=False)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # Create and run the event loop for async execution
    async def main():
        # Check if we should update or do full processing
        if len(sys.argv) > 1 and sys.argv[1] == "--update":
            await update_feature_engineering()
        else:
            await process_all_coins()
